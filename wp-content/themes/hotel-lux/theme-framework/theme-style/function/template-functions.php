...
/* Post Type Video */
function hotel_lux_post_type_video($cmsmasters_id, $video_type = '', $video_link = '', $video_links = '', $size = 'post-thumbnail') {
	if (!post_password_required()) {
		if ($video_type == 'selfhosted' && !empty($video_links) && sizeof($video_links) > 0) {
			...
			hotel_lux_thumb($cmsmasters_id, $size, true, false, true, false, true, true, false);
		}
	}
}
...
/* Post Type Slider */
function hotel_lux_post_type_slider($cmsmasters_id, $images, $size = 'post-thumbnail', $slider_data = '') {
	if (!post_password_required()) {
		if (sizeof($images) > 1) {
			echo '<div' . 
			...
			hotel_lux_thumb($cmsmasters_id, $size, false, 'img_' . $cmsmasters_id, true, true, true, true, $images[0]);
		} elseif (has_post_thumbnail()) {
			hotel_lux_thumb($cmsmasters_id, $size, false, 'img_' . $cmsmasters_id, true, true, true, true, false);
		}
	}
}
...
/* Post Type Gallery */
function hotel_lux_post_type_gallery($cmsmasters_id, $images, $columns, $size_col = 'post-thumbnail', $size_full = 'post-thumbnail') {
	if (!post_password_required()) {
		...
		if (sizeof($images) > 0 && $images[0] != '') {
			...
			hotel_lux_thumb($cmsmasters_id, $size_full, false, 'img_' . $cmsmasters_id, true, true, false, true, false);
		}
	}
}
...
/* Post Type Image */
function hotel_lux_post_type_image($cmsmasters_id, $image_link = '', $size = 'cmsmasters-masonry-thumb') {
	if (!post_password_required()) {
		if ($image_link != '') {
			hotel_lux_thumb($cmsmasters_id, $size, false, 'img_' . $cmsmasters_id, false, false, false, true, $image_link);
		} elseif (has_post_thumbnail()) {
			hotel_lux_thumb($cmsmasters_id, $size, false, 'img_' . $cmsmasters_id, false, false, false, true, false);
		}
	}
}
...
/* Post Type Audio */
function hotel_lux_post_type_audio($audio_links) {
...
function hotel_lux_get_header_search_form($cmsmasters_option) {
	if (
		...
		$out_form = "<form method=\"get\" action=\"" . esc_url(home_url('/')) . "\">
			<div class=\"cmsmasters_header_search_form_field\">
				...
				<input type=\"search\" name=\"s\" placeholder=\"" . esc_attr__('Search...', 'hotel-lux') . "\" value=\"\" />
			</div>
		</form>";
		
		
		$out_form = apply_filters('hotel_lux_get_header_search_form_filter', $out_form);
...
/* Theme Error Styles */
function hotel_lux_theme_error_styles() {
	$out = "";
	...
	if (is_404()) {
		...
		if (
			$cmsmasters_option['hotel-lux' . '_error_bg_img_enable'] && 
			$cmsmasters_option['hotel-lux' . '_error_bg_image'] != ''
		) {
			$error_bg_image = explode('|', $cmsmasters_option['hotel-lux' . '_error_bg_image']);
			...
			$out .= "
	.error .error_bg {
		background-image : " . (!empty($cmsmasters_option['hotel-lux' . '_error_bg_image']) ? 'url(' . ((is_numeric($error_bg_image[0])) ? $error_bg_image_url[0] : $error_bg_image[1]) . ')' : 'none') . ";
		background-position : " . (!empty($cmsmasters_option['hotel-lux' . '_error_bg_pos']) ? $cmsmasters_option['hotel-lux' . '_error_bg_pos'] : 'top center') . ";
		background-repeat : " . (!empty($cmsmasters_option['hotel-lux' . '_error_bg_rep']) ? $cmsmasters_option['hotel-lux' . '_error_bg_rep'] : 'repeat') . ";
		background-attachment : " . (!empty($cmsmasters_option['hotel-lux' . '_error_bg_att']) ? $cmsmasters_option['hotel-lux' . '_error_bg_att'] : 'scroll') . ";
		background-size : " . (!empty($cmsmasters_option['hotel-lux' . '_error_bg_size']) ? $cmsmasters_option['hotel-lux' . '_error_bg_size'] : 'auto') . ";
	}
";
		}
		...
		$out .= "
	.error .error_bg {
		" . cmsmasters_color_css('background-color', $cmsmasters_option['hotel-lux' . '_error_bg_color']) . "
	}
	
	.error .error_title, 
	.error .error_subtitle {
		" . cmsmasters_color_css('color', $cmsmasters_option['hotel-lux' . '_error_color']) . "
	}
";
	}
...
add_action('wp_enqueue_scripts', 'hotel_lux_theme_error_styles');



/* Get Page Heading Function */
function hotel_lux_page_heading() {
	if (is_404() || is_home()) {
		...
		if (
			...
			$cmsmasters_header_overlaps = $cmsmasters_option['hotel-lux' . '_header_overlaps'] ? 'true' : 'false';
			
			$cmsmasters_heading_alignment = $cmsmasters_option['hotel-lux' . '_heading_alignment'];
			$cmsmasters_heading_scheme = $cmsmasters_option['hotel-lux' . '_heading_scheme'];
			
			$cmsmasters_breadcrumbs = $cmsmasters_option['hotel-lux' . '_breadcrumbs'] ? 'true' : 'false';
		}
				...
				if ($cmsmasters_heading == 'custom') {
					...
					if (is_search()) {
						...
						if (!empty($wp_query->found_posts)) {
							echo sprintf(esc_html(_n('%1$d search result for: %2$s', '%1$d search results for: %2$s', $wp_query->found_posts, 'hotel-lux')), $wp_query->found_posts, get_search_query());
						} else {
							echo sprintf(esc_html__('0 search results for: %s', 'hotel-lux'), get_search_query());
						}
					} elseif (is_archive()) {
						if (is_author()) {
							if (get_the_author_meta('first_name') != '' || get_the_author_meta('last_name') != '') {
								echo sprintf(esc_html__('Author: %1$s (%2$s %3$s)', 'hotel-lux'), '<span class="vcard">' . get_the_author() . '</span>', get_the_author_meta('first_name'), get_the_author_meta('last_name'));
							} else {
								echo sprintf(esc_html__('Author: %s', 'hotel-lux'), '<span class="vcard">' . get_the_author() . '</span>');
							}
						} else {
...
function hotel_lux_theme_heading_styles() {
	$cmsmasters_option = hotel_lux_get_global_options();
	...
	if (
		...
		$cmsmasters_heading_height = $cmsmasters_option['hotel-lux' . '_heading_height'];
		$cmsmasters_heading_bg_color = $cmsmasters_option['hotel-lux' . '_heading_bg_color'];
		$cmsmasters_heading_bg_img_enable = $cmsmasters_option['hotel-lux' . '_heading_bg_image_enable'] ? 'true' : 'false';
		$cmsmasters_heading_bg_img = $cmsmasters_option['hotel-lux' . '_heading_bg_image'];
		$cmsmasters_heading_bg_rep = $cmsmasters_option['hotel-lux' . '_heading_bg_repeat'];
		$cmsmasters_heading_bg_att = $cmsmasters_option['hotel-lux' . '_heading_bg_attachment'];
		$cmsmasters_heading_bg_size = $cmsmasters_option['hotel-lux' . '_heading_bg_size'];
	}
...
add_action('wp_enqueue_scripts', 'hotel_lux_theme_heading_styles');



/* Get Social Icons Styles Function */
function hotel_lux_theme_social_icons_styles() {
...
add_action('wp_enqueue_scripts', 'hotel_lux_theme_social_icons_styles');



/* Get Posts Thumbnail Function */
function hotel_lux_thumb($cmsmasters_id, $type = 'post-thumbnail', $link = true, $group = false, $preload = true, $highImg = false, $fullwidth = true, $show = true, $attachment = false, $unique = false, $link_icon = false, $placeholder_icon = 'cmsmasters_theme_icon_image') {
	$args = array( 
...
function hotel_lux_thumb_small($cmsmasters_id, $type = 'post', $w = 100, $h = 100, $show = true) {
...
function hotel_lux_theme_category_styles() {
	$out = "";
	
	
	if (CMSMASTERS_COLORED_CATEGORIES) {
		$args = hotel_lux_colored_categories_args();
...
add_action('wp_enqueue_scripts', 'hotel_lux_theme_category_styles');



/* Get Page Container Classes */
function hotel_lux_get_page_classes($cmsmasters_option, $classes = false) {
	$browser = new Browser();
	...
	echo 'cmsmasters_' . $cmsmasters_option['hotel-lux' . '_theme_layout'] . ' ';
	
	
	if ($cmsmasters_option['hotel-lux' . '_fixed_header']) {
		echo 'fixed_header ';
	}
	...
	if ($cmsmasters_option['hotel-lux' . '_header_styles'] == 'r_nav') {
	...
	if ($cmsmasters_header_overlaps == '') {
		$cmsmasters_header_overlaps = $cmsmasters_option['hotel-lux' . '_header_overlaps'];
	}
...
/* Get Logo Function */
function hotel_lux_logo_small() {
	$cmsmasters_option = hotel_lux_get_global_options();
	
	
	if ($cmsmasters_option['hotel-lux' . '_logo_type'] == 'image') {
		list($logo_width, $logo_height) = getimagesize(get_template_directory() . '/theme-vars/theme-style' . CMSMASTERS_THEME_STYLE . '/img/logo_small.png');
		
		
		if ($cmsmasters_option['hotel-lux' . '_logo_url_small'] == '') {
			echo '<a href="' . esc_url(home_url('/')) . '" title="' . esc_attr(get_bloginfo('name')) . '" class="logo logo_small">' . "\n\t" . 
				'<img src="' . esc_url(get_template_directory_uri()) . '/theme-vars/theme-style' . CMSMASTERS_THEME_STYLE . '/img/logo_small.png" alt="' . esc_attr(get_bloginfo('name')) . '" />' . "\n\t" . 
				'<img class="logo_retina logo_small" src="' . esc_url(get_template_directory_uri()) . '/theme-vars/theme-style' . CMSMASTERS_THEME_STYLE . '/img/logo_small_retina.png" alt="' . esc_attr(get_bloginfo('name')) . '" width="' . $logo_width . '" height="' . $logo_height . '" />' . "\r" . 
			'</a>' . "\n";
		} else {
			$logo_img = explode('|', $cmsmasters_option['hotel-lux' . '_logo_url_small']);
			...
			if ($cmsmasters_option['hotel-lux' . '_logo_url_retina_small'] != '') {
				$logo_img_retina = explode('|', $cmsmasters_option['hotel-lux' . '_logo_url_retina_small']);
				...
				echo '<img class="logo_retina logo_small" src="' . 
				...
				echo '<img class="logo_retina logo_small" src="' . ((is_numeric($logo_img[0])) ? esc_url($logo_img_url[0]) : esc_url($logo_img[1])) . '" alt="' . esc_attr(get_bloginfo('name')) . '" />' . "\r";
			}
...
/* Theme Header Styles */
function hotel_lux_theme_header_styles() {
	$cmsmasters_option = hotel_lux_get_global_options();
	
	$defaults = hotel_lux_settings_general_defaults();
	
	
	$header_top_height = (($cmsmasters_option['hotel-lux' . '_header_top_height'] !== '') ? $cmsmasters_option['hotel-lux' . '_header_top_height'] : $defaults[$tab]['hotel-lux' . '_header_top_height']);
	...
	$header_bot_height = (($cmsmasters_option['hotel-lux' . '_header_bot_height'] !== '') ? $cmsmasters_option['hotel-lux' . '_header_bot_height'] : $defaults[$tab]['hotel-lux' . '_header_bot_height']);
...
";
	...
	if ($cmsmasters_option['hotel-lux' . '_custom_css'] != '') {
		$out .= stripslashes($cmsmasters_option['hotel-lux' . '_custom_css']);
	}
	
	wp_add_inline_style('hotel-lux-style', $out);
}

add_action('wp_enqueue_scripts', 'hotel_lux_theme_header_styles');
