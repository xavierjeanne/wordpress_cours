...
function hotel_lux_prev_next_posts($order_cat = false, $tax_name = 'category') {
	...
	if ($published_posts > 1) {
		...
		previous_post_link('<span class="cmsmasters_prev_post"><span class="post_nav_sub">' . esc_html__('Previous', 'hotel-lux') . '<span class="post_nav_type"> ' . (($cmsmasters_post_type == 'project') ? esc_html__('room', 'hotel-lux') : $cmsmasters_post_type) . ' </span>' . esc_html__('Link', 'hotel-lux') . '</span>%link<span class="cmsmasters_prev_arrow"><span></span></span></span>', '%title', $order_cat, '', $tax_name);
		
		next_post_link('<span class="cmsmasters_next_post"><span class="post_nav_sub">' . esc_html__('Next', 'hotel-lux') . '<span class="post_nav_type"> ' . (($cmsmasters_post_type == 'project') ? esc_html__('room', 'hotel-lux') : $cmsmasters_post_type) . ' </span>' . esc_html__('Link', 'hotel-lux') . '</span>%link<span class="cmsmasters_next_arrow"><span></span></span></span>', '%title', $order_cat, '', $tax_name);
...
function hotel_lux_sharing_box($title_box = false, $tag = 'h3') {
...
function hotel_lux_author_box($title_box = false, $tag = 'h3', $author_tag = 'h4') {
	...
	if ($user_description) {
		...
		if ($user_url) {
			echo '<a href="' . esc_url($user_url) . '" title="' . esc_attr(get_the_author()) . ' ' . esc_attr__('website', 'hotel-lux') . '" target="_blank">' . esc_html($user_url) . '</a>';
		}
...
function hotel_lux_related($tag = 'h3', $title = '', $no_title = '', $box_type = false, $tgsarray = null, $items_number = 5, $pause_time = 5, $type = 'post', $taxonomy = null) {
	if ( 
		...
		if ($r_query->have_posts()) {
			echo "<aside class=\"cmsmasters_single_slider\">" . 
				"<" . esc_html($tag) . " class=\"cmsmasters_single_slider_title\">" . 
					($title != '' ? esc_html($title) : esc_html__('More items', 'hotel-lux')) . 
				"</" . esc_html($tag) . ">" . 
						...
						while ($r_query->have_posts()) : $r_query->the_post();
							echo "<div class=\"cmsmasters_owl_slider_item cmsmasters_single_slider_item\">
								<div class=\"cmsmasters_single_slider_item_outer\">";
									...
									hotel_lux_thumb(get_the_ID(), 'cmsmasters-blog-masonry-thumb', true, false, true, false, true, true, false, false, false, 'cmsmasters_theme_icon_image');
									
									echo "<div class=\"cmsmasters_single_slider_item_inner\">";
									
										if ($type == 'post') {
											echo "<abbr class=\"published\" title=\"" . esc_attr(get_the_date()) . "\">" . esc_html(get_the_date()) . "</abbr>";
										}
...
function hotel_lux_author_avatar($template_type = 'page') {
...
function hotel_lux_get_pings($id, $tag = 'h3') {
	$out = '';
	...
	if (sizeof($pings) > 0) {
		$out .= '<aside class="cmsmasters_pings_list">' . "\n" .
			'<' . esc_html($tag) . '>' . esc_html__('Trackbacks and Pingbacks', 'hotel-lux') . '</' . esc_html($tag) . '>' . "\n" .
