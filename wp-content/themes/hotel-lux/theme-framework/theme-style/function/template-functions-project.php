...
function hotel_lux_project_heading($cmsmasters_id, $tag = 'h1', $link_redirect = false, $link_url = false, $link_target = false, $show = true) { 
...
function hotel_lux_project_title_nolink($cmsmasters_id, $tag = 'h1', $show = true) { 
...
function hotel_lux_project_exc_cont($show = true) {
...
function hotel_lux_project_check_exc_cont() {
	$exc = hotel_lux_project_exc_cont(false);
...
function hotel_lux_get_project_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			...
			if ($cmsmasters_option['hotel-lux' . '_portfolio_project_cat']) {
				$out .= '<div class="cmsmasters_project_category">' . 
					hotel_lux_get_the_category_list($cmsmasters_id, $taxonomy, '', '', '', true) . 
				'</div>';
			}
		}
	}
...
function hotel_lux_get_project_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['hotel-lux' . '_portfolio_project_like']) {
			$out = '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Likes', 'hotel-lux') . ':' . '</div>' . 
...
function hotel_lux_get_project_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = hotel_lux_get_comments('cmsmasters_project_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = hotel_lux_get_global_options();
			
			if ($cmsmasters_option['hotel-lux' . '_portfolio_project_comment'] && comments_open()) {
				$out = '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Comments', 'hotel-lux') . ':' . '</div>' . 
					'<div class="project_details_item_desc details_item_desc_comments">' . 
						hotel_lux_get_comments('cmsmasters_project_comments') . 
					'</div>' . 
				'</div>';
			}
		}
...
function hotel_lux_get_project_date($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['hotel-lux' . '_portfolio_project_date']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Date', 'hotel-lux') . ':' . '</div>' . 
...
function hotel_lux_get_project_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['hotel-lux' . '_portfolio_project_author']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Author', 'hotel-lux') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
					'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Projects by', 'hotel-lux') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author" rel="author"><span class="fn">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
				'</div>' . 
			'</div>';
...
function hotel_lux_get_project_tags($cmsmasters_id, $taxonomy, $show = true) {
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		$cmsmasters_option = hotel_lux_get_global_options();
		$out = '';
		
		if ($cmsmasters_option['hotel-lux' . '_portfolio_project_tag']) {
			$out = '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Tags', 'hotel-lux') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
...
function hotel_lux_get_project_features($features_position = 'features', $features = '', $features_title = false, $tag = 'h2', $show = true) {
	if (
		(
			!empty($features[0][0]) || 
			!empty($features[0][1])
...
function hotel_lux_project_link($link_text, $link_url, $link_target, $show = true) {
	$cmsmasters_option = hotel_lux_get_global_options();
	$out = '';
	...
	if ( 
		...
		$out = '<div class="project_details_item">' . 
			'<div class="project_details_item_title">' . esc_html__('Room Link', 'hotel-lux') . ':' . '</div>' . 
			'<div class="project_details_item_desc">' . 
...
function cmsmasters_project_more($cmsmasters_id, $show = true) {
	...
	if ($cmsmasters_project_read_more == '') {
		$cmsmasters_project_read_more = esc_html__('Learn More', 'hotel-lux');
	}
...
/* Get Projects Price Function */
function hotel_lux_project_price($price_text, $show = true) {
	$out = '';
	
	if ($price_text != '') {
		$out = '<span class="cmsmasters_project_price">' . esc_html($price_text) . '</span>';
	}
...
function hotel_lux_project_subtitle($subtitle, $show = true) {
	$out = '';
	
	if ($subtitle != '') {
		$out = '<h3 class="cmsmasters_project_subtitle">' . esc_html($subtitle) . '</h3>';
	}
