...
$more_text = ($more_text != '') ? $more_text : esc_html__('Load More Posts', 'hotel-lux');


$filter_text = ($filter_text != '') ? $filter_text : esc_html__('Filter', 'hotel-lux');


$filter_cats_text = ($filter_cats_text != '') ? $filter_cats_text : esc_html__('All Categories', 'hotel-lux');
	...
	while ($query->have_posts()) : $query->the_post();
		if ($layout == 'columns') {
			if ($layout_mode == 'puzzle') {
				$out .= cmsmasters_composer_ob_load_template('theme-framework/theme-style/postType/blog/post-puzzle.php', $this->blog_atts);
			} else {
				$out .= cmsmasters_composer_ob_load_template('theme-framework/theme-style/postType/blog/post-masonry.php', $this->blog_atts);
			}
		} elseif ($layout == 'timeline') {
			$out .= cmsmasters_composer_ob_load_template('theme-framework/theme-style/postType/blog/post-timeline.php', $this->blog_atts);
		} else {
			$out .= cmsmasters_composer_ob_load_template('theme-framework/theme-style/postType/blog/post-default.php', $this->blog_atts);
		}
	endwhile;
