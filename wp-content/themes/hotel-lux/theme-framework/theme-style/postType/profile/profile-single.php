...
if (
	$cmsmasters_option['hotel-lux' . '_profile_post_cat'] || 
	$cmsmasters_option['hotel-lux' . '_profile_post_comment'] || 
	(
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_open_profile'); ?>>
	...
	if ($cmsmasters_profile_title == 'true') {
		echo '<header class="cmsmasters_profile_header entry-header">';
			hotel_lux_profile_title_nolink(get_the_ID(), 'h2', $cmsmasters_profile_subtitle, 'h4');
		echo '</header>';
	}
		...
		echo '<div class="profile_sidebar">';
			...
			if ($profile_details == 'true') {
				...
				echo '<div class="profile_details entry-meta">';
					...
					hotel_lux_get_profile_likes('post');
					
					hotel_lux_get_profile_comments('post');
					
					hotel_lux_get_profile_features('details', $cmsmasters_profile_features, false, 'h3', true);
					
					hotel_lux_get_profile_category(get_the_ID(), 'pl-categs', 'post');
					
				echo '</div>';
			}
			
			
			hotel_lux_get_profile_features('features', $cmsmasters_profile_features_one, $cmsmasters_profile_features_one_title, 'h5', true);
			
			hotel_lux_get_profile_features('features', $cmsmasters_profile_features_two, $cmsmasters_profile_features_two_title, 'h5', true);
			
			hotel_lux_get_profile_features('features', $cmsmasters_profile_features_three, $cmsmasters_profile_features_three_title, 'h5', true);
			
			
			hotel_lux_profile_open_social_icons($cmsmasters_profile_social, get_the_ID(), esc_html__('Socials', 'hotel-lux'), 'h5');
...
if ($cmsmasters_profile_sharing_box == 'true') {
	hotel_lux_sharing_box(esc_html__('Like this profile?', 'hotel-lux'));
}


if ($cmsmasters_option['hotel-lux' . '_profile_post_nav_box']) {
	$order_cat = (isset($cmsmasters_option['hotel-lux' . '_profile_post_nav_order_cat']) ? $cmsmasters_option['hotel-lux' . '_profile_post_nav_order_cat'] : false);
	
	hotel_lux_prev_next_posts($order_cat, 'pl-categs');
}
