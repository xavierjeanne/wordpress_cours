...
if (!function_exists('hotel_lux_system_fonts_list')) {
	function hotel_lux_system_fonts_list() {
		$fonts = array( 
...
if (!function_exists('hotel_lux_text_decoration_list')) {
	function hotel_lux_text_decoration_list() {
		$list = array( 
			'none' => esc_html__('none', 'hotel-lux'), 
			'underline' => esc_html__('underline', 'hotel-lux'), 
			'overline' => esc_html__('overline', 'hotel-lux'), 
			'line-through' => esc_html__('line-through', 'hotel-lux'), 
		);
...
if (!function_exists('hotel_lux_custom_color_schemes_list')) {
	function hotel_lux_custom_color_schemes_list() {
		$list = array( 
			'first' => esc_html__('Custom 1', 'hotel-lux'), 
			'second' => esc_html__('Custom 2', 'hotel-lux'), 
			'third' => esc_html__('Custom 3', 'hotel-lux') 
		);
		
		
		return apply_filters('hotel_lux_custom_color_schemes_list_filter', $list);
	}
}
...
if (!function_exists('hotel_lux_locate_template')) {
	function hotel_lux_locate_template($template_names, $require_once = true, $load = true) {
		$located = '';
...
// Theme Style
define('CMSMASTERS_THEME_STYLE', (CMSMASTERS_THEME_STYLE_COMPATIBILITY && get_option('cmsmasters_hotel-lux_theme_style') ? get_option('cmsmasters_hotel-lux_theme_style') : ''));
...
if (!function_exists('hotel_lux_all_theme_styles')) {
	function hotel_lux_all_theme_styles() {
		$out = array( 
			'Default|', 
			'Theme Style 1|1', 
			'Theme Style 2|2' 
...
if (!function_exists('hotel_lux_all_color_schemes_list')) {
	function hotel_lux_all_color_schemes_list() {
		$list = array( 
			'default' => 		esc_html__('Default', 'hotel-lux'), 
			'header' => 		esc_html__('Header', 'hotel-lux'), 
			'navigation' => 	esc_html__('Navigation', 'hotel-lux'), 
			'header_top' => 	esc_html__('Header Top', 'hotel-lux'), 
			'footer' => 		esc_html__('Footer', 'hotel-lux') 
		);
		
		
		$out = array_merge($list, hotel_lux_custom_color_schemes_list());
...
if (!function_exists('hotel_lux_load_theme_textdomain')) {
	function hotel_lux_load_theme_textdomain() {
		load_theme_textdomain('hotel-lux', CMSMASTERS_VARS . '/languages');
	}
}

// Load Theme Local File Action
if (!has_action('after_setup_theme', 'hotel_lux_load_theme_textdomain')) {
	add_action('after_setup_theme', 'hotel_lux_load_theme_textdomain');
}



// Framework Activation & Data Import
if (!function_exists('hotel_lux_theme_activation')) {
	function hotel_lux_theme_activation() {
		if (get_option('cmsmasters_active_theme') != 'hotel-lux') {
			add_option('cmsmasters_active_theme', 'hotel-lux', '', 'yes');
...
add_action('after_switch_theme', 'hotel_lux_theme_activation');



// Framework Deactivation
if (!function_exists('hotel_lux_theme_deactivation')) {
	function hotel_lux_theme_deactivation() {
		delete_option('cmsmasters_active_theme');
	}
}

add_action('switch_theme', 'hotel_lux_theme_deactivation');



// Plugin Activation Regenerate Styles
if (!function_exists('hotel_lux_plugin_activation')) {
	function hotel_lux_plugin_activation($plugin, $network_activation) {
...
add_action('activated_plugin', 'hotel_lux_plugin_activation', 10, 2);


if (!function_exists('hotel_lux_plugin_activation_regenerate')) {
	function hotel_lux_plugin_activation_regenerate() {
...
add_action('init', 'hotel_lux_plugin_activation_regenerate');


function hotel_lux_run_reinit_import_options($post_id, $key, $value) {
	if (!get_post_meta($post_id, 'cmsmasters_heading', true)) {
		$custom_post_meta_fields = hotel_lux_get_custom_all_meta_fields();
...
add_action('import_post_meta', 'hotel_lux_run_reinit_import_options', 10, 3);
