...
<div class="tribe-mini-calendar-event event-<?php echo esc_attr( $mini_cal_event_atts['current_post'] ); ?> <?php echo esc_attr( $mini_cal_event_atts['class'] ); ?>">
				...
				<div class="cmsmasters_widget_event_venue_loc">
					<?php if ( isset( $organizer ) && $organizer && ! empty( $organizer_ids ) ): ?>
						<span class="tribe-events-organizer">
							...
							$and = _x( 'and', 'list separator for final two elements', 'hotel-lux' );
							if ( 1 == count( $organizer_links ) ) {
								echo hotel_lux_return_content($organizer_links[0]);
							}// end if
							elseif ( 2 == count( $organizer_links ) ) {
								echo hotel_lux_return_content($organizer_links[0] . ' ' . esc_html( $and ) . ' ' . $organizer_links[1]);
							}// end elseif
							else {
								...
								echo esc_html( ', ' . $and . ' ' );
								echo hotel_lux_return_content($last_organizer);
							}// end else
							?>
						</span>
					<?php endif ?>
	...
	<div class="cmsmasters_events_more">
		<a href="<?php echo esc_url( tribe_get_event_link() ); ?>" class="tribe-events-read-more button" rel="bookmark"><?php esc_html_e( 'Find out more', 'hotel-lux' ) ?></a>
	</div>
</div>
