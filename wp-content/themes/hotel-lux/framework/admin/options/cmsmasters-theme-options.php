...
if (!function_exists('hotel_lux_get_custom_all_meta_fields')) {
function hotel_lux_get_custom_all_meta_fields() {
	$custom_meta_fields = hotel_lux_get_custom_general_meta_fields();
	...
	if ( 
		...
		$custom_page_meta_fields = hotel_lux_get_custom_page_meta_fields();
		...
		$custom_project_meta_fields = hotel_lux_get_custom_project_meta_fields();
		...
		$custom_profile_meta_fields = hotel_lux_get_custom_profile_meta_fields();
		...
		$custom_post_meta_fields = hotel_lux_get_custom_post_meta_fields();
		
		$custom_all_meta_fields = array_merge($custom_post_meta_fields, $custom_meta_fields);
	} else {
		$custom_other_meta_fields = hotel_lux_get_custom_other_meta_fields();
...
function hotel_lux_admin_enqueue_scripts($hook) {
	if ( 
		($hook == 'post.php') || 
		($hook == 'post-new.php') 
	) {
		...
		wp_enqueue_style('hotel-lux-options-css', get_template_directory_uri() . '/framework/admin/options/css/cmsmasters-theme-options.css', array(), '1.0.0', 'screen');
		
		
		if (is_rtl()) {
			wp_enqueue_style('hotel-lux-options-css-rtl', get_template_directory_uri() . '/framework/admin/options/css/cmsmasters-theme-options-rtl.css', array(), '1.0.0', 'screen');
		}
		
		
		wp_enqueue_script('hotel-lux-options-js', get_template_directory_uri() . '/framework/admin/options/js/cmsmasters-theme-options.js', array('jquery'), '1.0.0', true);
		
		wp_localize_script('hotel-lux-options-js', 'cmsmasters_options', array( 
			'palettes' => 			implode(',', cmsmasters_color_picker_palettes()), 
			'create_gallery' => 	esc_attr__('Create Gallery', 'hotel-lux'), 
			'select_format' => 		esc_attr__('Please select the format.', 'hotel-lux'), 
			'link_exists' => 		esc_html__('Link with this format already exists.', 'hotel-lux'), 
			'want_remove' => 		esc_html__('Do you really want to remove this item?', 'hotel-lux'), 
		...
		wp_enqueue_script('hotel-lux-options-js-toggle', get_template_directory_uri() . '/framework/admin/options/js/cmsmasters-theme-options-toggle.js', array('jquery'), '1.0.0', true);
	}
}

add_action('admin_enqueue_scripts', 'hotel_lux_admin_enqueue_scripts');


function hotel_lux_show_meta_box() {
	global $post;
	
	$custom_all_meta_fields = hotel_lux_get_custom_all_meta_fields();
	...
	foreach ($custom_all_meta_fields as $field) {
		...
		case 'select_sidebar':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Default Sidebar', 'hotel-lux') . '</option>';
		...
		case 'select_slider':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Slider', 'hotel-lux') . '</option>';
		...
		case 'select_post_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Blog Category', 'hotel-lux') . '</option>';
		...
		case 'select_project_cat':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Project Category', 'hotel-lux') . '</option>';
		...
		case 'select_pl_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Profile Category', 'hotel-lux') . '</option>';
		...
		case 'icon':
			echo '<div class="icon_management">' . 
				'<p>' . 
					...
					'<input id="' . $field['id'] . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'hotel-lux') . '" />' . 
					'<a href="#" class="cmsmasters_remove_icon admin-icon-remove' . (($meta != '') ? ' cmsmasters_dib' : '') . '" title="' . esc_attr__('Remove icon', 'hotel-lux') . '"></a>' . 
				'</p>' . 
			'</div>' . 
		...
		case 'contact':
			...
			'<div class="contact_info_management">' . 
				'<p>' . 
					...
					'<input id="' . $field['id'] . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Add info', 'hotel-lux') . '" />' . 
					'<a href="#" class="cmsmasters_remove_icon cmsmasters_remove_icon_contact_info admin-icon-remove" title="' . esc_attr__('Cancel changes', 'hotel-lux') . '"></a>' . 
				'</p>' . 
				...
				'</span>' . 
				'<span class="cl"></span>' . 
				'<input class="button button-primary" type="button" id="add_contact_info" value="' . esc_attr__('Add Contact Info', 'hotel-lux') . '" />' . 
				'<input class="button button-primary" type="button" id="edit_contact_info" value="' . esc_attr__('Save Contact Info', 'hotel-lux') . '" />' . 
				'<ul>';
		...
		case 'repeatable_link':
			...
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'hotel-lux') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Link', 'hotel-lux') . '</strong></div>' . 
			'</div>' . 
			...
			'<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<optgroup label="' . esc_attr__('Blank Field', 'hotel-lux') . '">' . 
					'<option value="">' . esc_html__('Select Link', 'hotel-lux') . '</option>' . 
				'</optgroup>' . 
				'<optgroup label="' . esc_attr__('Posts', 'hotel-lux') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Pages', 'hotel-lux') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Projects', 'hotel-lux') . '">';
		...
		case 'repeatable_multiple':
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'hotel-lux') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Values', 'hotel-lux') . '</strong></div>' . 
			'</div>' . 
			'<ul id="' . $field['id'] . '-repeatable" class="custom_repeatable">';
		...
		case 'repeatable_media':
			echo '<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<option value="">' . esc_html__('Select Format', 'hotel-lux') . ' &nbsp;</option>';
		...
		case 'images_list':
			...
			echo '<div class="cmsmasters_upload_parent cmsmasters_gallery_parent">' . 
				'<input type="button" id="cmsmasters_gallery_' . $field['id'] . '_button" class="cmsmasters_upload_button button button-large" value="' . (($meta != '') ? esc_attr__('Edit Gallery', 'hotel-lux') : esc_attr__('Create Gallery', 'hotel-lux')) . '" data-title="' . esc_attr__('Create/Edit Gallery', 'hotel-lux') . '" data-button="' . esc_attr__('Insert Gallery', 'hotel-lux') . '" data-id="cmsmasters-media-select-frame-' . $field['id'] . '" data-classes="media-frame cmsmasters-media-gallery-frame' . ((!isset($field['description'])) ? ' cmsmasters-frame-no-description' : '') . ((!isset($field['caption'])) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($field['align'])) ? ' cmsmasters-frame-no-align' : '') . ((!isset($field['link'])) ? ' cmsmasters-frame-no-link' : '') . ((!isset($field['size'])) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $field['frame'] . '"' . (($field['frame'] == 'post') ? ' data-state="' . (($meta != '') ? 'gallery-edit' : 'gallery-library') . '"' : '') . ' data-multiple="' . $field['multiple'] . '"' . (($meta != '') ? ' data-editing="true"' : '') . ' />' . 
				...
				foreach ($ids as $id) {
					if (isset($id[0]) && isset($id[1])) {
						echo '<li class="cmsmasters_gallery_item">' . 
							...
							'<a href="#" class="cmsmasters_gallery_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'hotel-lux') . '"></a>' . 
						'</li>';
					}
				}
			}
		...
		case 'funds':
			...
			echo '<div class="cmsmasters_funds_stats">' . 
				'<div class="cmsmasters_funds_stat_wrap">' . 
					...
					'</div>' . 
				'</div>' . 
				'<span class="fl">' . esc_html__('count', 'hotel-lux') . ' - <strong>' . $funds_number . '</strong></span>' . 
				'<span class="fr">' . cmsmasters_donations_currency(cmsmasters_number_format($target - $funds)) . ' ' . esc_html__('to go', 'hotel-lux') . '</span>';
			'</div>' . 
			(($field['desc'] != '') ? '<br />' . '<span class="description">' . $field['desc'] . '</span>': '');
...
function hotel_lux_save_custom_meta($post_id) {
    $custom_all_meta_fields = hotel_lux_get_custom_all_meta_fields();
...
add_action('save_post', 'hotel_lux_save_custom_meta');


function hotel_lux_add_custom_meta_box() {
	$args = array(
	   'public' => true 
	);
	...
	foreach ($screens as $screen) {	
		add_meta_box( 
			...
			esc_html__('Theme Options', 'hotel-lux'), 
			'hotel_lux_show_meta_box', 
			$screen, 
			'normal', 
			'high' 
		);
...
add_action('add_meta_boxes', 'hotel_lux_add_custom_meta_box');
