...
function hotel_lux_get_admin_page() {
	global $pagenow;
...
function hotel_lux_default_tab() {
	$current_page = hotel_lux_get_admin_page();
...
function hotel_lux_get_the_tab() {
	global $pagenow;
	
	$default_tab = hotel_lux_default_tab();
...
function hotel_lux_settings_page_header() {
	...
	if ($tabs != '') {
		...
		if (class_exists('Cmsmasters_Custom_Fonts') && $page == 'cmsmasters-settings-font') {
			$links[] = '<a class="' . $class . ' cmsmasters_recaptcha_nav_tab' . '" href="edit.php?post_type=cmsmasters_font">' . esc_html__('Custom Fonts', 'hotel-lux') . '</a>';
		}
