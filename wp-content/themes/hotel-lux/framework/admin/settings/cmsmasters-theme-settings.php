...
add_action('admin_menu', 'hotel_lux_add_menu');
add_action('admin_init', 'hotel_lux_register_settings');
...
function hotel_lux_get_settings() {
	$output = array();
	
	
	$page = hotel_lux_get_admin_page();
	...
	case 'cmsmasters-settings':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'hotel-lux' . CMSMASTERS_THEME_STYLE;
		$cmsmasters_settings_page_title = esc_html__('Theme General Settings', 'hotel-lux');
		$cmsmasters_page_sections = hotel_lux_options_general_sections();
		$cmsmasters_page_fields = hotel_lux_options_general_fields();
		$cmsmasters_page_tabs = hotel_lux_options_general_tabs();
	...
	case 'cmsmasters-settings' . '-font':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'hotel-lux' . CMSMASTERS_THEME_STYLE . '_font';
		$cmsmasters_settings_page_title = esc_html__('Theme Fonts Settings', 'hotel-lux');
		$cmsmasters_page_sections = hotel_lux_options_font_sections();
		$cmsmasters_page_fields = hotel_lux_options_font_fields();
		$cmsmasters_page_tabs = hotel_lux_options_font_tabs();
	...
	case 'cmsmasters-settings' . '-color':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'hotel-lux' . CMSMASTERS_THEME_STYLE . '_color';
		$cmsmasters_settings_page_title = esc_html__('Theme Color Schemes', 'hotel-lux');
		$cmsmasters_page_sections = hotel_lux_options_color_sections();
		$cmsmasters_page_fields = hotel_lux_options_color_fields();
		$cmsmasters_page_tabs = hotel_lux_options_color_tabs();
	...
	case 'cmsmasters-settings' . '-element':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'hotel-lux' . CMSMASTERS_THEME_STYLE . '_element';
		$cmsmasters_settings_page_title = esc_html__('Theme Elements Settings', 'hotel-lux');
		$cmsmasters_page_sections = hotel_lux_options_element_sections();
		$cmsmasters_page_fields = hotel_lux_options_element_fields();
		$cmsmasters_page_tabs = hotel_lux_options_element_tabs();
	...
	case 'cmsmasters-settings' . '-single':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'hotel-lux' . CMSMASTERS_THEME_STYLE . '_single';
		$cmsmasters_settings_page_title = esc_html__('Theme Single Posts Settings', 'hotel-lux');
		$cmsmasters_page_sections = hotel_lux_options_single_sections();
		$cmsmasters_page_fields = hotel_lux_options_single_fields();
		$cmsmasters_page_tabs = hotel_lux_options_single_tabs();
	...
	case 'cmsmasters-settings' . '-demo':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'hotel-lux' . CMSMASTERS_THEME_STYLE . '_demo';
		$cmsmasters_settings_page_title = esc_html__('Theme Settings Import/Export', 'hotel-lux');
		$cmsmasters_page_sections = hotel_lux_options_demo_sections();
		$cmsmasters_page_fields = hotel_lux_options_demo_fields();
		$cmsmasters_page_tabs = hotel_lux_options_demo_tabs();
...
function hotel_lux_create_settings_field($args = array()) {
	$defaults = array( 
		'id' => 		'default_field', 
		'title' => 		esc_html__('Default Field', 'hotel-lux'), 
		'desc' => 		esc_html__('This is a default description.', 'hotel-lux'), 
		'std' => 		'', 
		'type' => 		'text', 
...
function hotel_lux_register_settings() {
	$settings_output = hotel_lux_get_settings();
	...
	register_setting($cmsmasters_option_name, $cmsmasters_option_name, 'hotel_lux_validate_options');
	
	
	if (!empty($settings_output['cmsmasters_page_sections'])) {
		foreach ($settings_output['cmsmasters_page_sections'] as $id => $title) {
			add_settings_section($id, $title, 'hotel_lux_section_fn', __FILE__);
		}
	}
	...
	if (!empty($settings_output['cmsmasters_page_fields'])) {
		foreach ($settings_output['cmsmasters_page_fields'] as $option) {
			hotel_lux_create_settings_field($option);
		}
	}
...
function hotel_lux_settings_scripts() {
	wp_enqueue_style('wp-jquery-ui-dialog');
	
	
	wp_enqueue_style('hotel-lux-settings-css', get_template_directory_uri() . '/framework/admin/settings/css/cmsmasters-theme-settings.css', array(), '1.0.0', 'screen');
	
	
	if (is_rtl()) {
		wp_enqueue_style('hotel-lux-settings-css-rtl', get_template_directory_uri() . '/framework/admin/settings/css/cmsmasters-theme-settings-rtl.css', array(), '1.0.0', 'screen');
	}
	...
	wp_enqueue_script('hotel-lux-settings-js', get_template_directory_uri() . '/framework/admin/settings/js/cmsmasters-theme-settings.js', array('jquery', 'farbtastic'), '1.0.0', true);
	
	
	$theme_style_name = '';
	
	if (CMSMASTERS_THEME_STYLE_COMPATIBILITY) {
		foreach (hotel_lux_all_theme_styles() as $theme_style) {
	...
	wp_localize_script('hotel-lux-settings-js', 'cmsmasters_setting', array( 
		'palettes' => 						implode(',', cmsmasters_color_picker_palettes()), 
		'remove' => 						esc_attr__('Remove', 'hotel-lux'), 
		'remove_sidebar' => 				esc_html__('Do you really want to remove this sidebar?', 'hotel-lux'), 
		'remove_google_web_font' => 		esc_html__('Do you really want to remove this google web font?', 'hotel-lux'), 
		'find' => 							esc_attr__('Find icons', 'hotel-lux'), 
		'remove_icon' => 					esc_html__('Do you really want to remove this social icon?', 'hotel-lux'), 
		'theme_uri' => 						get_template_directory_uri(), 
		'shortname' => 						'hotel-lux', 
		'theme_style_name' => 				$theme_style_name, 
		'done' => 							esc_html__('All theme settings were imported successfully.', 'hotel-lux'), 
		'fail' => 							esc_html__("Error on page!!!\nPlease reload page and try again.", 'hotel-lux'), 
		'done_reset_settings' => 			esc_html__('Theme style settings were reset to default.', 'hotel-lux'), 
	...
	wp_enqueue_script('hotel-lux-settings-js-toggle', get_template_directory_uri() . '/framework/admin/settings/js/cmsmasters-theme-settings-toggle.js', array('jquery'), '1.0.0', true);
	
	wp_localize_script('hotel-lux-settings-js-toggle', 'cmsmasters_settings', array( 
		'shortname' => 	'hotel-lux' 
	));
...
function hotel_lux_add_menu() {
	...
	$add_menu_p( 
		esc_attr__('Theme Settings', 'hotel-lux'), 
		esc_attr__('Theme Settings', 'hotel-lux'), 
		'manage_options', 
		'cmsmasters-settings', 
		'hotel_lux_settings_page_fn', 
		'' 
	);
	...
	$cmsmasters_settings_general = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme General Settings', 'hotel-lux'), 
		esc_attr__('General', 'hotel-lux'), 
		'manage_options', 
		'cmsmasters-settings', 
		'hotel_lux_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_general, 'hotel_lux_settings_scripts');
	
	
	$cmsmasters_settings_font = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Fonts Settings', 'hotel-lux'), 
		esc_attr__('Fonts', 'hotel-lux'), 
		'manage_options', 
		'cmsmasters-settings' . '-font', 
		'hotel_lux_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_font, 'hotel_lux_settings_scripts');
	
	
	$cmsmasters_settings_color = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Color Schemes', 'hotel-lux'), 
		esc_attr__('Colors', 'hotel-lux'), 
		'manage_options', 
		'cmsmasters-settings' . '-color', 
		'hotel_lux_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_color, 'hotel_lux_settings_scripts');
	
	
	$cmsmasters_settings_element = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Elements Settings', 'hotel-lux'), 
		esc_attr__('Elements', 'hotel-lux'), 
		'manage_options', 
		'cmsmasters-settings' . '-element', 
		'hotel_lux_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_element, 'hotel_lux_settings_scripts');
	
	
	$cmsmasters_settings_single = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Single Posts Settings', 'hotel-lux'), 
		esc_attr__('Single Posts', 'hotel-lux'), 
		'manage_options', 
		'cmsmasters-settings' . '-single', 
		'hotel_lux_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_single, 'hotel_lux_settings_scripts');
	
	
	$cmsmasters_settings_demo = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Settings Import/Export', 'hotel-lux'), 
		esc_attr__('Import/Export', 'hotel-lux'), 
		'manage_options', 
		'cmsmasters-settings' . '-demo', 
		'hotel_lux_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_demo, 'hotel_lux_settings_scripts');
}


function hotel_lux_section_fn($desc) {
	$tab = hotel_lux_get_the_tab();
...
function hotel_lux_settings_page_fn() {
	$settings_output = hotel_lux_get_settings();
	$current_tab = hotel_lux_get_the_tab();
	
	echo '<div class="wrap">';
	
	hotel_lux_settings_page_header();
	...
	if ($current_tab != 'wpml') {
		echo '<form action="options.php" method="post" class="cmsmasters_admin_page' . (($current_tab == 'recaptcha') ? ' cmsmasters_admin_page_recaptcha' : '') . '">' . 
			'<p class="submit-top">' . 
				(($current_tab != 'import' && $current_tab != 'export' && $current_tab != 'theme_style') ? '<input name="submit-top" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'hotel-lux') . '" />' : '') . 
			'</p>';
			...
			echo '<p class="submit">';
				...
				if ($current_tab != 'import' && $current_tab != 'export' && $current_tab != 'theme_style') {
					echo '<input name="submit" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'hotel-lux') . '" />';
				} elseif ($current_tab == 'import') {
					echo '<input name="import" type="button" class="button button-primary button-large cmsmasters-demo-import" value="' . esc_attr__('Import Settings', 'hotel-lux') . '" />';
				} elseif ($current_tab == 'theme_style') {
					echo '<input name="apply-theme-style" type="submit" class="button button-primary button-large cmsmasters-apply-theme-style" value="' . esc_attr__('Apply This Style', 'hotel-lux') . '" />';
					echo '<input name="reset-settings" type="button" class="button button-large cmsmasters-reset-settings" value="' . esc_attr__('Reset Theme Style Settings to Default', 'hotel-lux') . '" />';
				}
...
function hotel_lux_form_field_fn($args = array()) {
	extract($args);
	...
	case 'select_theme_style':
		$theme_style_option = get_option('cmsmasters_hotel-lux_theme_style');
		
		echo '<select id="' . $id . '" class="select' . $field_class . '" name="cmsmasters_hotel-lux_theme_style">';
	...
	case 'upload':
		...
		echo '<div class="cmsmasters_upload_parent cmsmasters_select_parent">' . 
			'<input type="button" id="cmsmasters_upload_' . $id . '_button" class="cmsmasters_upload_button button button-large" value="' . esc_attr__('Choose Image', 'hotel-lux') . '" data-title="' . esc_attr__('Choose Image', 'hotel-lux') . '" data-button="' . esc_attr__('Insert Image', 'hotel-lux') . '" data-id="cmsmasters-media-select-frame-' . $id . '" data-classes="media-frame cmsmasters-media-select-frame' . ((!isset($description)) ? ' cmsmasters-frame-no-description' : '') . ((!isset($caption)) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($align)) ? ' cmsmasters-frame-no-align' : '') . ((!isset($link)) ? ' cmsmasters-frame-no-link' : '') . ((!isset($size)) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $frame . '"' . (($frame == 'post') ? ' data-state="insert"' : '') . ' data-multiple="' . $multiple . '" />' . 
			'<div class="cmsmasters_upload' . (($image != '') ? ' cmsmasters_db' : '') . '">' . 
				'<img src="' . (($image != '') ? $image : '') . '" class="cmsmasters_preview_image" />' . 
				'<a href="#" class="cmsmasters_upload_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'hotel-lux') . '"></a>' . 
			'</div>' . 
	...
	case 'typorgaphy':
		...
		if ($system_font) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (hotel_lux_system_fonts_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_system_font'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_system_font">' . esc_html__('System Font', 'hotel-lux') . '</label>' . 
			'</div>';
		}
			...
			echo '<div class="cmsmasters_admin_block">' . 
				'<select class="select" id="' . $id . '_google_font" name="' . $cmsmasters_option_name . '[' . $id . '_google_font]">';
					...
					if (!empty($cmsmasters_fonts_list['local'])) {
						echo '<optgroup label="' . esc_attr__('Local Fonts', 'hotel-lux') . '">';
					...
					if (!empty($cmsmasters_fonts_list['web'])) {
						echo '<optgroup label="' . esc_attr__('Google Web Fonts', 'hotel-lux') . '">';
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_google_font">' . esc_html__('Google Font', 'hotel-lux') . '</label>' . 
			'</div>';
		}
		...
		if ($font_size) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_font_size_number">' . esc_html__('Font Size', 'hotel-lux') . ' <em>(' . esc_html__('in pixels', 'hotel-lux') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($line_height) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_line_height_number">' . esc_html__('Line Height', 'hotel-lux') . ' <em>(' . esc_html__('in pixels', 'hotel-lux') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($font_weight) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_weight">' . esc_html__('Font Weight', 'hotel-lux') . '</label>' . 
			'</div>';
		}
		...
		if ($font_style) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_style">' . esc_html__('Font Style', 'hotel-lux') . '</label>' . 
			'</div>';
		}
		...
		if ($text_transform) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_transform">' . esc_html__('Text Transform', 'hotel-lux') . '</label>' . 
			'</div>';
		}
		...
		if ($text_decoration) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (hotel_lux_text_decoration_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_text_decoration'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_decoration">' . esc_html__('Text Decoration', 'hotel-lux') . '</label>' . 
			'</div>';
		}
	...
	case 'sidebar':
		...
		'<div class="sidebar_management">' . 
			'<p>' . 
				...
				'<input class="button" type="button" id="add_sidebar" value="' . esc_attr__('Add Sidebar', 'hotel-lux') . '" />' . 
			'</p>' . 
			'<div></div>' . 
			'<ul>';
	...
	case 'google_web_fonts':
		...
		'<div class="google_web_fonts_manager">' . 
				...
				if (isset($options[$id]) && is_array($options[$id])) {
					...
					foreach($options[$id] as $font) {
						...
						echo '<div class="google_web_fonts_manager_item">' . 
							'<input class="all-options" type="text" id="google_web_font_family" placeholder="' . esc_attr__('Font Family (for example: Roboto Slab)', 'hotel-lux') . '" name="' . $cmsmasters_option_name . '[' . $id . '_-_' . $i . '_family]" value="' . esc_attr($font[1]) . '" />' . 
							'<input class="regular-text" type="text" id="google_web_font_style" placeholder="' . esc_attr__('Font Style (for example: Roboto+Slab:100,300,400,700)', 'hotel-lux') . '" name="' . $cmsmasters_option_name . '[' . $id . '_-_' . $i . '_style]" value="' . esc_attr($font[0]) . '" />' . 
			...
			'<input class="button" type="button" id="add_google_web_font" value="' . esc_attr__('Add Font', 'hotel-lux') . '" data-font-family-label="' . esc_attr__('Font Family (for example: Roboto Slab)', 'hotel-lux') . '" data-font-style-label="' . esc_attr__('Font Style (for example: Roboto+Slab:100,300,400,700)', 'hotel-lux') . '" data-option-name="' . $cmsmasters_option_name . '[' . $id . '_-_" />' . 
		'</div>';
	...
	case 'social':
		...
		'<div class="icon_management">' . 
			'<p>' . 
				...
				'<input id="' . $id . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'hotel-lux') . '" />' . 
				'<a href="#" class="cmsmasters_remove_icon admin-icon-remove" title="' . esc_attr__('Cancel changes', 'hotel-lux') . '"></a>' . 
			'</p>' . 
			'<span class="cl"><br /></span>' . 
			'<span class="icon_upload_link cmsmasters_dn">' . 
				'<label for="new_icon_color">' . 
					'<span class="cmsmasters_col_label">' . esc_html__('Icon Color', 'hotel-lux') . '</span>' . 
					'<div>' . 
				...
				'<label for="new_icon_hover">' . 
					'<span class="cmsmasters_col_label">' . esc_html__('Icon Hover Color', 'hotel-lux') . '<br /><em>' . esc_html__('if not selected - Icon Color will be used', 'hotel-lux') . '</em></span>' . 
					'<div>' . 
				...
				'<label for="new_icon_link">' . 
					...
					esc_html__('Icon Link', 'hotel-lux') . 
				'</label>' . 
				'<label for="new_icon_title">' . 
					'<input class="all-options" type="text" id="new_icon_title" /> ' . 
					esc_html__('Icon Title', 'hotel-lux') . 
				'</label>' . 
				'<label for="new_icon_target">' . 
					'<input type="checkbox" id="new_icon_target" value="true" /> ' . 
					esc_html__('Open link in a new tab/window?', 'hotel-lux') . 
				'</label>' . 
			'</span>' . 
			'<span class="cl"></span>' . 
			'<input class="button button-primary" type="button" id="add_icon" value="' . esc_attr__('Add Icon', 'hotel-lux') . '" />' . 
			'<input class="button button-primary" type="button" id="edit_icon" value="' . esc_attr__('Save Icon', 'hotel-lux') . '" />' . 
			'<ul>';
			...
			if (isset($options[$id]) && is_array($options[$id])) {
				foreach($options[$id] as $icon) {
					...
					echo '<li>' . 
						...
						'</div>' . 
						'<a href="#" class="icon_del admin-icon-remove" title="' . esc_attr__('Remove', 'hotel-lux') . '"></a> ' . 
...
function hotel_lux_validate_options($input) {
	$valid_input = array();
	...
	foreach ($options as $option) {
		...
		case 'text':
			...
			case 'numeric':
				...
				$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Expecting a Numeric value!', 'hotel-lux');
				
				if (is_numeric($input[$option['id']]) == false) {
					add_settings_error(
						$option['id'],
						'hotel-lux' . '_txt_numeric_error',
						esc_html__('Expecting a Numeric value! Please fix.', 'hotel-lux'),
						'error'
					);
				}
			...
			case 'multinumeric':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : esc_html__('Expecting comma separated numeric values', 'hotel-lux');
				} else {
				...
				if ($input[$option['id']] != '' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
					add_settings_error(
						$option['id'],
						'hotel-lux' . '_txt_multinumeric_error',
						esc_html__('Expecting comma separated numeric values! Please fix.','hotel-lux'),
						'error'
					);
				}
			...
			case 'email':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (is_email($input[$option['id']]) !== false) ? $input[$option['id']] : esc_html__('Invalid email! Please re-enter!', 'hotel-lux');
				} elseif ($input[$option['id']] == '') {
					$valid_input[$option['id']] = esc_html__('This setting field cannot be empty! Please enter a valid email address.', 'hotel-lux');
				}
				
				if (is_email($input[$option['id']]) == false || $input[$option['id']] == '') {
					add_settings_error(
						$option['id'],
						'hotel-lux' . '_txt_email_error',
						esc_html__('Please enter a valid email address.', 'hotel-lux'),
						'error'
					);
				}
		...
		case 'number':
			$input[$option['id']] = trim($input[$option['id']]);
			$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Number!', 'hotel-lux');
...
function hotel_lux_show_msg($message, $msgclass = 'info') {
	echo '<div id="message" class="' . $msgclass . '">' . $message . '</div>';
}


function hotel_lux_admin_msgs() {
	global $pagenow;
...
function hotel_lux_add_global_options($reset = false) {
	...
	foreach (hotel_lux_all_color_schemes_list() as $key => $value) {
		array_push($cmsmasters_option_names, array( 
			'cmsmasters_options_' . 'hotel-lux' . CMSMASTERS_THEME_STYLE . '_color_' . $key, 
			hotel_lux_options_color_fields($key) 
		));
	}
		...
		if ($cmsmasters_option_name[1] !== '') {
			foreach ($cmsmasters_option_name[1] as $selected_option) {
				if ( 
					...
					$selected_option['id'] !== 'hotel-lux' . '_social_icons' && 
					$selected_option['id'] !== 'hotel-lux' . '_google_web_fonts' 
				) {
...
function hotel_lux_get_global_options() {
	...
	foreach (hotel_lux_all_color_schemes_list() as $key => $value) {
		array_push($cmsmasters_option_names, 'cmsmasters_options_' . 'hotel-lux' . CMSMASTERS_THEME_STYLE . '_color_' . $key);
	}
